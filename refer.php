...
if ($length > 1) while ($i < $length - 1)
{
	$j += CONF_COL;
	printf("<tr>\n");
	if ($i == 0) printf(get_item(htmlentities($directory), $i, "file-symlink-directory.svg", $index, "[Parent]", dirname($directory)."/", "?path="));
...
else printf(get_item(htmlentities($directory), $i, "file-symlink-directory.svg", $index, "[Parent]", dirname($directory)."/", "?path=")."<td>\n</td>\n<td>\n</td>\n<td>\n</td>\n</tr>\n");
